...
CACHE_TTL_SEC = 7200
# Десять воркеров вместо шести: замер дал 225 c против 280 c при тех же 16749
# людях и 45 программах. Прирост непропорционален росту потоков (+67% потоков →
# −20% времени), то есть упирается уже не в наш параллелизм, а в скорость, с
# которой fa.ru отдаёт страницу. Поэтому выше не поднимаем: платить риском
# троттлинга за проценты не стоит. Недокачать молча ФА не может — число страниц
# считается из поля total самого сайта, и сбой любой страницы после ретраев
# роняет сборку с откатом на прошлый кэш.
MAX_WORKERS = 10
FETCH_RETRIES = 3
...
# Квотных недоборов у ФА мы не моделируем — не потому, что данных нет, а потому
# что модель на них не работает. Квотные списки есть (type_conkurs: «Отдельная
# квота», «Имеющие право на льготное зачисление», «Поступающие по целевому
# направлению»), и недобор по ним огромен: на квоты подают по одному человеку
# при 8–12 местах. Но вуз возвращает в общий конкурс лишь малую часть — замер по
# отслеживаемым программам дал предикт 77/111/23 места против 59/78/21, которые
# сайт реально зачисляет. Подбирать долю возврата под наблюдения означало бы
# выдумать число, поэтому места остаются строго по приказу о КЦП.
