...
# «вас нет в списках» вводит в заблуждение: робот считает ТОЛЬКО бюджетное
# поступление, и человека, подавшегося на платные направления, он не увидит.
# У МИРЭА это видно прямо в API: тип конкурса 4 — бюджетный, 6 — платный (так
# их метит и сам фронтенд вуза). Замер 10.08.2026: из 97 очных программ
# бюджетный конкурс есть у 32, у остальных 65 в выдаче только платный.
_NOT_FOUND_HINTS = {
    "МИРЭА": (
        "Робот считает только бюджетный конкурс. У МИРЭА бюджетные списки есть "
        "у 32 очных направлений из 97 — на остальных вуз публикует только "
        "платный конкурс, и в бюджетных списках таких абитуриентов нет."
    ),
}
