...
def _rows_and_meta_from_bacc(html: str) -> tuple[list[dict], int | None, int | None, int | None]:
    ...
    * **«Количество вакантных мест»** — сколько основных мест ещё свободно
      прямо сейчас. С 10.08.2026 это ОСНОВНОЙ источник мест для каскада (см.
      сборку программ в `build`): плановый КЦП завышал число мест там, где часть
      уже разобрана на прошлых этапах. Цена — сверка по МЭИ перестала быть
      независимой: вакантных мест ровно столько же, сколько людей отмечено
      «Высшим проходным», то есть число мест частично и есть ответ сайта.
      Решение принято владельцем осознанно: точность прогноза важнее
      доказательности сверки.
...
class MpeiFullPool:
    ...
    def _fetch_all(
        ...
        for title, list_id in catalog:
            ...
            # Места — «Количество вакантных мест» со страницы волны: сколько мест
            # РЕАЛЬНО разыгрывается сейчас. Плановый КЦП общего конкурса для
            # прогноза не годится: часть мест уже разобрана на прошлых этапах, и
            # каскад раздаёт несуществующие. Живой пример — «Фундаментальная
            # информатика»: КЦП общего конкурса 13, вакантных 6, и робот на
            # лишнем месте пропускал балл 288 туда, где последний прошедший имел
            # 292.
            #
            # ЦЕНА РЕШЕНИЯ (принята сознательно, по требованию владельца): в
            # числе вакантных мест уже частично зашит ответ сайта — их ровно
            # столько же, сколько людей отмечено «Высшим проходным». Поэтому
            # сверка прогноза по МЭИ больше НЕ независима, и её "match" не
            # является доказательством правильности модели. Оракул остаётся
            # полезен как индикатор расхождений, но не как доказательство.
            #
            # Квотные недоборы отдельно НЕ добавляем: вуз уже вернул незанятые
            # квотные места в общий конкурс, и в вакантных они учтены (у ИВТ КЦП
            # 139, наш расчётный недобор даёт 169, вакантных 170). Добавить их
            # сверху — посчитать дважды, поэтому при живых вакантных
            # quota_shortfall гасится и оптимистичный прогноз не строится.
            group = kcp_groups.get(kcp_group_title(title))
            vacant = vacant_by_list.get(list_id)
            shortfall = (shortfalls or {}).get(title)
            if vacant is not None:
                places, seat_source, shortfall = vacant, "live", None
