...
# Квоты, по которым спрашиваем места отдельно. «Целевая квота» здесь не значится
# намеренно: kcp.php отдаёт по ней null, а список пуст — целевой приём СТАНКИН
# машинно не раскрывает.
QUOTA_TYPES = ("Особая квота", "Отдельная квота")
...
CACHE_TTL_SEC = 7200
# Два воркера, а не шесть: сайт троттлит пагинацию и при шести молча обрывает
...
# кто не проходит. Страховка от остаточного троттлинга — детектор обрыва на
# полной странице в fetch_direction_rows.
MAX_WORKERS = 2
