...
# Тип конкурса «Основной бюджет» на сайте СТАНКИНа. Значение именно "-", а не
# текст пункта: в <select name="PROPERTY_402"> у него value="-", и бэкенд
# принимает только его (на строку «Основной бюджет» отдаёт пустой список).
MAIN_BUDGET = "-"

BASE_PARAMS = {
    "PROPERTY_388": "Бюджетная основа",
    "PROPERTY_389": "1 - Очная",
    # Без этого фильтра список приезжает по ВСЕМ бюджетным конкурсам сразу —
    # общий конкурс вместе с особой, отдельной и целевой квотами. Замер на
    # «27.03.01 Стандартизация и метрология»: без фильтра 819 строк / 781 код,
    # с фильтром 774 / 771, и лишние 10 человек — чистые квотники, которых в
    # общем конкурсе нет. Места мы при этом всегда брали по общему конкурсу
    # (kcp.php), то есть конкуренты и места приезжали из РАЗНЫХ конкурсов:
    # робот сажал квотников на общие места и был пессимистичнее вуза.
    "PROPERTY_402": MAIN_BUDGET,
    "PROPERTY_584": "ready",
    "LIST_TYPE": "ranked",
    "EDU_LEVEL": "bs",
...
_SEAT_PARAMS = {
    "PROPERTY_388": "Бюджетная основа",
    "PROPERTY_389": "1 - Очная",
    # Тот же тип конкурса, что и у списков (см. BASE_PARAMS): места и конкуренты
    # обязаны приходить из одного конкурса, иначе робот считает одних людей на
    # места других.
    "PROPERTY_402": MAIN_BUDGET,
    "EDU_LEVEL": "bs",
}
...
CACHE_TTL_SEC = 7200
# Три воркера, а не шесть: сайт троттлит пагинацию и при шести молча обрывает
# списки на середине. Замер на «09.03.01 Информатика и вычислительная техника»:
# шесть воркеров дают 227 строк, последовательный обход — 3476. Обрыв
# бесшумный (200 OK без ссылки «Следующая»), поэтому цена не в скорости, а в
# том, что робот считает по неполному списку конкурентов и раздаёт места тем,
# кто не проходит.
#
# Значение подобрано замером, а не на глаз. Четыре прогона на одном каталоге:
# 2 воркера — 655 c, 3 — 459 c, 5 — 308 c, 8 — 204 c, и ВСЕ дали одинаковые
# 45732 строки, цифра в цифру по каждому из 21 направления, без единой ошибки.
#
# Утренняя поломка на шести воркерах (589 человек вместо 8255) была ДО фильтра
# PROPERTY_402: списки тогда приезжали по всем бюджетным конкурсам сразу и были
# заметно толще, страниц качалось больше. После сужения до общего конкурса сайт
# держит восемь потоков спокойно. Если троттлинг вернётся (например в час пик),
# сработает детектор обрыва в fetch_direction_rows: направление перекачается, а
# при повторном обрыве сборка упадёт и откатится на прошлый кэш — тихо испортить
# данные он не даст.
