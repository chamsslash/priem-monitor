...
def build_robot_settings(code: str, university: str) -> RobotSettings:
    ...
    без обращения к config/robot.json. dima_score здесь безопасный дефолт:
    в ветке "код найден в пуле" simulator.py берёт настоящий балл прямо
    из пула (found.score), это поле не используется. dima_consent=True —
    ИНАЧЕ: simulator.py::_resolve_dima_person всегда подставляет именно
    это значение вместо реального согласия найденного человека из пула
    (это существующее поведение симулятора, не баг этой функции — то же
    самое уже было для Димы через старый config/robot.json, где
    dima.consent тоже был захардкожен). Практическое следствие: если
    реальный пользователь НЕ давал согласия на зачисление, а
    require_consent=True (дефолт simulation), симуляция всё равно
    покажет его как потенциально проходящего. Известное ограничение,
    унаследованное от simulator.py (который в этом плане не меняется),
    не исправляется здесь."""
